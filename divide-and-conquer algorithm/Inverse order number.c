...
long long inverse(int *a,int s,int e){
    if(s>=e){
        return 0;
    }
    int mid=s+(e-s)/2;
    long long num;
    num=inverse(a,s,mid)+inverse(a,mid+1,e);
    sort(a,s,mid);
    sort(a,mid+1,e);
    int i=s,j=mid+1;
    while(i<=mid && j<=e){
...
比如可以通过某个人对一些事物的排名来估计他（或她）对各种不同信息的兴趣，
从而实现个性化的服务。

对于不同的排名结果可以用逆序来评价它们之间的差异。
考虑1,2,…,n的排列i1，i2，…，in，如果其中存在j,k，
满足 j < k 且 ij > ik， 那么就称(ij,ik)是这个排列的一个逆序。
